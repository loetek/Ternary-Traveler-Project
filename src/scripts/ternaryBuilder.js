...
const ternary = {
    ...
    displayTernary(chosenOne)
    {
       ...
       //!!Creation of the return to landing page button.
        const homeButton = domComponents.createDomElement({ elementType:"a",cssClass:"homeButton", content:"HOME", attributes:{src:`${traveler}`, id:"homeButton"}});
        output.appendChild(homeButton);
        homeButton.addEventListener("click", this.welcome);
        ...
        .then(r => {
            ...
            r.forEach(rDetails =>{
                ...
                const cityName = domComponents.createDomElement({ elementType:"h2",cssClass:"cityTitle", content:`${rDetails.name}`, attributes:{id:"cityTitle"}});
       ...
       // console.log(interestCard);
        ...
        .then(rInterests => {
            ...
            rInterests.forEach(intDetails =>{
                ...
                let cityContainer = domComponents.createDomElement({ elementType:"fieldset", cssClass:"fieldsets", attributes:{ id:`${intDetails.place.id}`}, tabIndex:`${intDetails.place.id}` });
                ...
                let editLocation = domComponents.createDomElement({elementType:"button", cssClass:"button", content: "Edit Entry", attributes:{ id:`editButton--${intDetails.id}`}});
